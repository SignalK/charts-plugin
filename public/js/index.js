...
async function setupCharts() {
  ...
  jobsControl.onAdd = function () {
    ...
    btn.innerHTML =
      '<img src="leaflet/images/downloads.png" alt="Active seeding jobs" style="width:16px;height:16px;">'
  ...
  helpControl.onAdd = function () {
    ...
    btn.innerHTML =
      '<img src="leaflet/images/lifebuoy.png" alt="Help" style="width:16px;height:16px;">'
  ...
  map.on('mousemove', function (e) {
    ...
    document.querySelector('.map-status-control').innerHTML =
    ...
    entries
      ...
      .forEach(([id, info]) => {
        if (info.proxy === true) {
          ...
          let layer = L.tileLayer(`${info.url}`, {
  ...
  map.on('overlayremove', function (e) {
    ...
    settings.selections.charts = settings.selections.charts.filter(
      (c) => c !== name
    )
...
function createJobCard(job) {
  ...
  } else if (job.type === JobType.Delete) {
...
function updateRegionButtons() {
  ...
  const p = activeRegionLayer.feature.properties
  const value = p.name || p.Name || ''
  const createBtn = document.getElementById('createJobButton')
